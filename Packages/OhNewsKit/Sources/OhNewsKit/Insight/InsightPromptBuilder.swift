...
public enum InsightPromptBuilder {
    ...
    public static func systemPrompt() -> String {
        ...
        你写的是一块小面板上的导语，**越短越好**：读者扫一眼就去读正文了，
        多写一句就是多挡一行正文。宁可少说，也不要凑字数。
        ...
        字段要求（都是上限，写得比这更短更好）：
        - article_summary：一句话，不超过 50 字，说明正文讲了什么。只能依据给出的正文作答，
          不要补充正文里没有的背景，也不要评价写得如何。
        - key_points：最多 3 条，每条不超过 18 字。要具体，不要写成「文章讨论了……」
          这种什么都没有的话。
        - discussion_summary：一句话，不超过 40 字，概括评论区的核心观点。
          没有评论区时填空字符串。
        - discussion_trends：最多 2 条，每条不超过 18 字，写评论区值得注意的分歧、共识
          或情绪走向。没有评论区时填空数组。不要把个别评论当成整体趋势。
        """
    }
