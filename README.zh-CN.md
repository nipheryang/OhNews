...
OhNews 是以 **MIT 许可证**发布的自由开源软件，可以自由使用、修改与再分发，包括商业用途。详见 [`LICENSE`](LICENSE)。
...
OhNews 以 **MIT 许可证**开源（[`LICENSE`](LICENSE)）：可以自由使用、修改、再分发，包括用于商业项目与闭源产品。唯一的要求是保留版权声明与许可文本。

欢迎贡献代码，贡献按同一许可接受，见 [`CONTRIBUTING.md`](CONTRIBUTING.md)。
