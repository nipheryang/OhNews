...
HeyNews 是一款原生 macOS 新闻阅读应用，当前支持 Hacker News，提供可选的中文 AI 摘要与应用内正文阅读。Hacker News 是它接入的第一个信息源。

它面向这样一类使用方式：快速浏览 Hacker News，理解一条内容背后的讨论，并直接打开原文，而不必在多个浏览器标签之间来回切换。

HeyNews 是专有的 source-available 产品，使用与分发须遵守 [`LICENSE`](LICENSE) 与 [`EULA.md`](EULA.md)。

## 安装

HeyNews 以 macOS 磁盘映像（DMG）形式分发，要求 macOS 15.0 或更高版本。

1. 打开下载得到的 `.dmg` 文件。
2. 将 `HeyNews.app` 拖入 `Applications` 文件夹。
3. 从 `Applications` 或 Spotlight 启动 HeyNews。
...
AI 是可选功能。未配置服务商时，HeyNews 仍是一个完整可用的 Hacker News 阅读器。
...
密钥存放于 macOS 钥匙串，不会写入 UserDefaults、仓库或应用日志。关闭 AI 或未配置服务商时，HeyNews 不会向任何 AI 服务商发送正文或评论内容。
...
之后用 Xcode 打开 `HeyNews.xcodeproj`，运行 `HeyNews` scheme。
...
在 `HeyNews/` 下新增或删除源文件后，先运行 `xcodegen generate` 再构建，以保证生成的 Xcode 工程同步。

## 许可与法律文件

HeyNews 为专有软件。使用或分发前请阅读 [`LICENSE`](LICENSE) 与 [`EULA.md`](EULA.md)。
